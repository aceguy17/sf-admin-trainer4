...
def quiz_get(request: Request):
    ...
    return templates.TemplateResponse(
        "quiz.html",
        {"request": request, "question": question, "question_number": idx + 1, "total_questions": total, "index": idx},
    )
...
def quiz_post(request: Request, index: int = Form(...), selected_option: int = Form(...)):
    ...
    return templates.TemplateResponse(
        "feedback.html",
        {"request": request, "feedback": feedback, "correct_answer": correct_answer, "explanation": explanation, "next_index": next_idx},
    )
...
def results(request: Request, total: int = 0, correct: int = -1):
    ...
    return templates.TemplateResponse(
        "results.html",
        {"request": request, "total": total_questions, "correct": "—", "score": score},
    )
...
def progress(request: Request):
    ...
    return templates.TemplateResponse(
        "progress.html",
        {"request": request, "total_answered": total_answered, "progress_items": sorted_items},
    )
